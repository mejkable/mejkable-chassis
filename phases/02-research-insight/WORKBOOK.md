# Phase 02 — Research & Insight: WORKBOOK

## Phase Plan

[Written by the Phase Plan prompt in PROMPT.md once the user has approved it. Sub-task selection lives here, never as ticks in BRIEF.md. Update the Status column as the phase runs.]

| Sub-task | Tier | Format | Why it applies to this project | Status |
|---|---|---|---|---|
| | Core / Conditional / Optional | | | planned / done / skipped |

**Skipped from the menu, and why:**
- 

**Approved by:** [user] on [date]

---

## How to Use

Capture all research outputs here. This phase typically generates the most material — keep it organised under clear headings. Raw data is fine. The clean summary goes in HANDOVER.md.

---

## Competitive Deep-Dive

[Paste analysis here]

### Competitor Matrix
| Competitor | Product | Price | Positioning | Strengths | Weaknesses | Distribution |
|---|---|---|---|---|---|---|
| | | | | | | |

### Positioning White Spaces
[Where is the opportunity to differentiate?]

---

## User Research Synthesis

[Paste synthesis here]

### Top Unmet Needs
1. 
2. 
3. 

### Top Valued Attributes
1. 
2. 
3. 

---

## Market Sizing

[Paste analysis and methodology here]

---

## Assumption Validation Tracker

| Assumption | Original Status | Evidence Found | Updated Status | Implication |
|---|---|---|---|---|
| | | | | |

---

## Materials & Technology

[Paste findings here]

---

## Manufacturing Methods

[Paste findings here]

---

## Regulatory & Compliance

[Paste findings here]

### Critical Requirements Summary
[List the non-negotiable regulatory requirements]

---

## Channel & Distribution

[Paste findings here]

---

## Pricing Research

[Paste findings here]

---

## Additional Research

[Add sections for any other sub-tasks you ran]

---

## Critique

[Output of the Critique prompt, run in a fresh session after the draft HANDOVER exists. Gaps, unsupported claims, the critic's confidence rating, and what was changed in response. Disagreements with the producer go in DECISIONS.md.]

---

## Raw Notes & Sources

[Links, source documents, expert quotes, data files]

