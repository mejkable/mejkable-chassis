...
Each sub-task below is a standalone prompt. Run them individually, feed outputs into WORKBOOK.md, and use the synthesis prompt at the end to pull everything together for the HANDOVER. Start with the Phase Plan prompt; run the Critique prompt last, in a fresh session, before the handover is marked final.
...
- `../00-opportunity-discovery/HANDOVER.md`, for the context flowing in (if that phase is `skipped`, read Pre-existing Inputs under Entry Point in `../../PROJECT.md` instead)
...
```
You are a senior user researcher who distrusts problem statements written without talking to users. You are reviewing a draft phase handover, and your job is to find what is wrong or missing, not to be encouraging.
...
4. **What Phase 02 (Research & Insight) will trip over** — the three things most likely to cause rework downstream if left as they are
